...
def download():
    ...
    if not os.path.exists(DATA_DIR):
        os.mkdir(DATA_DIR)
   # if not os.path.exists(os.path.join(DATA_DIR, 'shapenetcorev2_hdf5_2048')):
    if not os.path.exists(os.path.join(DATA_DIR, 'modelnet40_ply_hdf5_2048')):
        www = 'https://shapenet.cs.stanford.edu/media/shapenetcorev2_hdf5_2048.zip'
...
def load_data(partition):
    ...
    all_data = []
    all_label = []
   # for h5_name in glob.glob(os.path.join(DATA_DIR, 'shapenetcorev2_hdf5_2048', '%s*.h5' % partition)):
